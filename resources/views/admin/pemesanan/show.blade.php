    ...
    <div class="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
            <div class="flex items-center gap-3">
                <x-avatar-initial :name="$pemesanan->nama_tamu" />
                <div>
                    <h2 class="text-lg font-semibold text-slate-800">{{ $pemesanan->nama_tamu }}</h2>
                    <div class="text-xs text-slate-500">No. HP: {{ $pemesanan->no_hp_tamu }}</div>
                </div>
            </div>
        ...
        <div class="space-y-4">
            <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="text-xs font-medium text-slate-500">Status Booking</div>
                <div class="mt-1.5"><x-status-badge :status="$pemesanan->status" /></div>

                <div class="mt-4 text-xs font-medium text-slate-500">Sudah Dibayar</div>
                <div class="font-semibold tabular-nums text-slate-800">Rp{{ number_format($pemesanan->totalDibayar(), 0, ',', '.') }}</div>

                <div class="mt-3 text-xs font-medium text-slate-500">Sisa Tagihan</div>
                <div class="font-semibold tabular-nums {{ $pemesanan->sisaTagihan() > 0 ? 'text-red-600' : 'text-green-600' }}">
