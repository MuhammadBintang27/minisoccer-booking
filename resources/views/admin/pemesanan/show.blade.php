<x-layouts.admin title="Detail Pemesanan">
    ...
    <div class="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="rounded-xl bg-white p-6 shadow-sm lg:col-span-2">
            <h2 class="text-lg font-semibold text-slate-800">{{ $pemesanan->lapangan->nama }}</h2>
            <div class="mt-2 text-sm text-slate-600">
                ...
                <div class="mt-1">Nama: {{ $pemesanan->nama_tamu }}</div>
                <div>No. HP: {{ $pemesanan->no_hp_tamu }}</div>
            </div>

            <div class="mt-4 border-t border-slate-100 pt-4 text-sm text-slate-600">
                <div class="flex justify-between"><span>Slot</span><span>Rp{{ number_format($pemesanan->harga, 0, ',', '.') }}</span></div>
                @foreach ($pemesanan->layananTambahan as $addon)
                    <div class="flex justify-between"><span>{{ $addon->nama }}</span><span>Rp{{ number_format($addon->pivot->harga, 0, ',', '.') }}</span></div>
                @endforeach
                <div class="mt-2 flex justify-between border-t border-slate-100 pt-2 font-semibold text-slate-800">
                    <span>Total</span><span>Rp{{ number_format($pemesanan->totalHarga(), 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div class="mt-2 overflow-x-auto rounded-lg border border-slate-100">
                <table class="w-full text-left text-xs">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($pemesanan->pembayaran as $bayar)
                            <tr>
        ...
        <div class="space-y-4">
            <div class="rounded-xl bg-white p-6 shadow-sm">
                <div class="text-xs text-slate-500">Status Booking</div>
                <div class="mt-1 font-semibold text-slate-800">{{ \App\Support\StatusLabel::label($pemesanan->status) }}</div>

                <div class="mt-3 text-xs text-slate-500">Sudah Dibayar</div>
                <div class="font-semibold text-slate-800">Rp{{ number_format($pemesanan->totalDibayar(), 0, ',', '.') }}</div>

                <div class="mt-3 text-xs text-slate-500">Sisa Tagihan</div>
                <div class="font-semibold {{ $pemesanan->sisaTagihan() > 0 ? 'text-red-600' : 'text-green-600' }}">
                    Rp{{ number_format($pemesanan->sisaTagihan(), 0, ',', '.') }}
                </div>
            </div>
